...
static NSMutableArray<NSDictionary<NSString *, NSString *> *> *calcMemory = nil;
static NSMutableArray<NSDictionary<NSString *, NSString *> *> *calcMemoryRaw = nil;
 ...
 @interface MemoryViewController : UITableViewController
@property (nonatomic, assign) BOOL showingConversions;
@property (nonatomic, strong) UISegmentedControl *toggle;
@end
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"Memory";
    self.showingConversions = YES;

    self.toggle = [[UISegmentedControl alloc] initWithItems:@[@"Conversions", @"Standard"]];
    ...
    self.navigationItem.titleView = self.toggle;

    [self.tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"cell"];
}

- (void)toggleChanged:(UISegmentedControl *)sender {
    self.showingConversions = (sender.selectedSegmentIndex == 0);
    [self.tableView reloadData];
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return self.showingConversions ? calcMemory.count : calcMemoryRaw.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    NSDictionary *entry = self.showingConversions ? calcMemory[indexPath.row] : calcMemoryRaw[indexPath.row];

    NSString *left = self.showingConversions ? entry[@"input"] : entry[@"expression"];
    NSString *right = self.showingConversions ? entry[@"output"] : entry[@"result"];

    cell.textLabel.text = [NSString stringWithFormat:@"%@ → %@", left, right];
...
- (void)showMemory {
    if (!calcMemory || calcMemory.count == 0) return;
...
- (void)triggerConversion {
    ...
    if (!calcMemory) calcMemory = [[NSMutableArray alloc] init];

if (text.length > 0 && converted.length > 0) {
    NSDictionary *entry = @{ @"input": text, @"output": converted };

    NSDictionary *last = calcMemory.lastObject;
    if (![last isEqualToDictionary:entry]) {
        [calcMemory addObject:entry];
        if (calcMemory.count > 10)
            [calcMemory removeObjectAtIndex:0];

        NSLog(@"[CalcConv] 🧠 Added: %@ → %@", text, converted);
    }
}
...
if (text.length > 0 && converted.length > 0) {
    ...
    // Only add if different from the most recent entry
    NSDictionary *last = calcMemory.lastObject;
    if (![last isEqualToDictionary:entry]) {
        [calcMemory addObject:entry];
        if (calcMemory.count > 10)
...
UITapGestureRecognizer *doubleTap = [[UITapGestureRecognizer alloc] initWithTarget:helper action:@selector(reverseConversion)];
doubleTap.numberOfTapsRequired = 2;
[conversionLabel addGestureRecognizer:doubleTap];

// Long press to show memory
UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:helper action:@selector(showMemory)];
[conversionLabel addGestureRecognizer:longPress];

conversionLabel.userInteractionEnabled = YES;

// Associate helper with the label so it doesn't get released
objc_setAssociatedObject(conversionLabel, "_helper", helper, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
%hook CalculatorModelAccessibility

- (void)buttonPressed:(id)arg1 {
    %orig;

    NSString *key = [arg1 isKindOfClass:[NSString class]] ? arg1 : [arg1 valueForKey:@"key"];
    if (!key) return;

    NSLog(@"[CalcConv] 🧮 Pressed: %@", key);

    if ([key isEqualToString:@"EQUALS"]) {
        if (!calcMemoryRaw) calcMemoryRaw = [[NSMutableArray alloc] init];
        ...
        for (UIView *subview in window.rootViewController.view.subviews) {
            if ([NSStringFromClass([subview class]) containsString:@"DisplayView"]) {
                for (UIView *label in subview.subviews) {
                    if ([label isKindOfClass:[UILabel class]]) {
                        UILabel *disp = (UILabel *)label;
                        if (disp.text.length > 0) result = disp.text;

                        if ([disp respondsToSelector:@selector(accessibilityValue)]) {
                            expression = [disp performSelector:@selector(accessibilityValue)];
                        }
                    }
                }
            }
        ...
        if (expression && result) {
            NSDictionary *entry = @{ @"expression": expression, @"result": result };
            NSDictionary *last = calcMemoryRaw.lastObject;
            if (![last isEqualToDictionary:entry]) {
                [calcMemoryRaw addObject:entry];
                if (calcMemoryRaw.count > 10) [calcMemoryRaw removeObjectAtIndex:0];

                NSLog(@"[CalcConv] ➕ Standard Memory Added: %@ = %@", expression, result);
            }
        } else {
            NSLog(@"[CalcConv] ⚠️ Could not extract expression or result");
        }
    }
}
